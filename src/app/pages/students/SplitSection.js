...
function SplitSection(props) {
  const { title, text, img = null, bgColor = null, color = null, reverse = false, bottom = false } = props;
    ...
    <Box className="w-full flex justify-center">
      <Box className={`max-w-[1250px] w-full border-t ${bottom && 'border-b'} border-black flex flex-col md:flex-row`}>
        {!reverse && (
          ...
          >
            <Typography component="h2" sx={{ fontSize: 70, fontWeight: 400, lineHeight: 'normal' }}>
              {title}
            </Typography>
